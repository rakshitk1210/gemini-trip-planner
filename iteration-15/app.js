...
const GEMINI_KEY = 'AIzaSyDPEc5bauwJ50MEaDOIqeIquZeupgWIt4I';
...
async function callGemini(history) {
  ...
  const places = parseGeminiPlaces(raw);
...
async function animateSuggestions(promptText) {
  ...
  let places;
  try {
    places = await callGemini(conversationHistory);
  } catch (err) {
    console.error('Gemini error:', err);
    thinking.error('Couldn\'t reach Gemini — showing sample places.');
    places = SUGGESTIONS;
  }

  aiPlaces = places;
  conversationHistory.push({ role: 'model', text: JSON.stringify(places) });
...
async function submitFooter() {
  ...
  try {
    ...
    console.error('Gemini error:', err);
    thinking.error('Couldn\'t reach Gemini — try again.');
